...
class Vector extends Var {
    ...
    public Var add(Var other) throws CalcException{
        if (other instanceof Scalar) { //Это прибавление к каждому элементу вектора некоего числа. Математического смысла в этом нет
            ...
            double[] res = Arrays.copyOf(value, value.length);
            if (res.length!=((Vector) other).value.length) throw new CalcException("ERROR: операция невозможна с векторами разной длины.");
    ...
    public Var mul(Var other) throws CalcException{
        if (other instanceof Scalar) { //Это умножение числа на вектор. Математического смысл есть.
            ...
            if (Arrays.copyOf(value, value.length).length!=((Vector) other).value.length) throw new CalcException("ERROR: операция невозможна с векторами разной длины.");
